...
export default function Reminders() {
  ...
  const totalActive =
    today.filter((r) => !done[r.id]).length + week.length + later.length + overdue.length
    ...
    return (
      <div className="app-layout">
        ...
        <div className="main-content">
          <LoadingSpinner message="Reminders ophalen..." />
        </div>
      </div>
    )
  }
      ...
      <div className="main-content">
        ...
        <div className="reminders-content">
          ...
          {overdue.length > 0 && (
            <>
              <div className="reminders-section-header">
                ...
                <span className="section-title" style={{ color: 'var(--text-sec)' }}>
                  Verlopen
                </span>
              ...
              {overdue.map((rem) => {
                ...
                return (
                  <div
                    key={rem.id}
                    className="reminder-card done"
                    onClick={() => navigate(`/notitie/${rem.id}`)}
                  >
                    ...
                    <div
                      className="reminder-time-badge"
                      style={{ color: 'var(--text-sec)', background: 'var(--input-bg)' }}
                    >
                      <Bell size={12} />
          ...
          {today.length === 0 ? (
            ...
            today.map((rem) => {
              ...
              return (
                <div
                  key={rem.id}
                  className={`reminder-card${isDone ? ' done' : ''}`}
                  onClick={() => navigate(`/notitie/${rem.id}`)}
                >
                  <div
          ...
          {week.length === 0 ? (
            <div className="empty-state">
              <Circle size={26} color="var(--border)" />
              <span>Geen reminders deze week</span>
            </div>
          ) : (
            week.map((rem) => {
              const lblColor = rem.labels[0]?.color || 'var(--text-sec)'
